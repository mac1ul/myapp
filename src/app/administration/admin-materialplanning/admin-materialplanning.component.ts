import { Component, OnInit } from '@angular/core';
import { HttpClient } from '@angular/common/http';
import { DatePipe } from '@angular/common';
import * as fs from 'file-saver';
import * as XLSX from 'xlsx';
import { Workbook } from 'exceljs';
declare var $: any;
@Component({
  selector: 'app-admin-materialplanning',
  templateUrl: './admin-materialplanning.component.html',
  styleUrls: ['./admin-materialplanning.component.scss'],
  providers: [DatePipe]
})
export class AdminMaterialplanningComponent implements OnInit {
  country: any;
  solution_partner: any;
  project: any;
  machine: any;
  tabledata: any;
  excel : any;
  showpagination: boolean = false;
  showpartnerdetails: boolean = false;
  showpartner: boolean = false;
  showproject: boolean = false;
  showmachine: boolean = false;
  // variable for machine
  location: string = '';
  project_name: string = '';
  partner_company_name: string = '';
  machine_name: string = '';
  machine_phase: string = '';
  industry_sector: string = '';
  application: string = '';
  machine_description: string = '';
  toggleBool: boolean = false;
  company_id: string = '';

  rowsPerPage: number = 10;
  actualPage: number = 1;
  showprevnext: boolean = false;
  endPaging: any;
  showProducts: any;
  showerrmessage: any;
  showhideerrmsg: boolean = false;
  set_table_value_SPartner: string = '';
  set_table_value_project: string = '';
  set_table_value_machine: string = '';
  
  logoBase64 = "data:image/jpeg;base64,";
  latest_date1: any;
  constructor(private httpClient: HttpClient, private datePipe: DatePipe) {
    this.latest_date1 = this.datePipe.transform(new Date(), 'medium');
  }

  ngOnInit(): void {
    // All Country
    this.httpClient.get<any>("https://rb-mobileweb.de.bosch.com/2021/spm/php_v3.0/getallsalesunitdetails.php").subscribe((res: any) =>{
      this.country = res;
      // console.log(res);
    });


    this.pagination();
  }

  onchangecountry(country_name: any){
    // alert(country_name)
    if(country_name)
    {
      this.httpClient.get<any>("https://rb-mobileweb.de.bosch.com/2021/spm/php_v3.0/Material_Planning_SPofCU.php?country="+country_name).subscribe((res: any) =>{
      this.solution_partner = res;
      // console.log(res);
    });
    this.project = null;
    this.machine = null;
    }else{
      this.solution_partner = null;
      this.project = null;
      this.machine = null;
    }
  }

  onchangepartner(partner_id: any){
    if(partner_id)
    {
      this.httpClient.get<any>("https://rb-mobileweb.de.bosch.com/2021/spm/php_v3.0/Material_Planning_PROJECTofSP.php?company_id="+partner_id).subscribe((res: any) =>{
      this.project = res;
      //  console.log(res);
    });
    this.machine = null;
    }else{
      this.project = null;
      this.machine = null;
    }
  }


  onchangeproject(project: any){
    if(project)
    {
      this.httpClient.get<any>("https://rb-mobileweb.de.bosch.com/2021/spm/php_v3.0/Material_Planning_MACHINEofPROJECT.php?project_id="+project).subscribe((res: any) =>{
      this.machine = res;
      // console.log(res);
    });
    }else{
      this.machine = null;
    }
  }

  search(param: any, status: any){
    
    if(status === "partner"){
      // this.ngOnInit()

      this.httpClient.get<any>("https://rb-mobileweb.de.bosch.com/2021/spm/php_v3.0/admin_material_getsolutionpartnerbycountry.php?country="+param+"&status="+status).subscribe((res: any) =>{
      this.tabledata = res;
      this.showpartnerdetails = true;
      this.showpartner = false;
      this.showproject = false;
      this.showmachine = false;
      this.showhideerrmsg = false;
      this.pagination();
      this.updateViewAfterPaginationClicked();
      });
      this.showpagination = true;
    }else if(status === "partnerdetails"){

      this.set_table_value_SPartner = param;
      // alert(param)
      this.onchangepartner(param)

      if(param == null || param == ""){

      }else{
        this.httpClient.get<any>("https://rb-mobileweb.de.bosch.com/2021/spm/php_v3.0/admin_material_getsolutionpartnerbycountry.php?partnerdetails="+param+"&status="+status).subscribe((res: any) =>{
      this.tabledata = res;
      if(this.tabledata == null || this.tabledata == ""){
        this.showhideerrmsg = true;
        this.showpartner = false;
        this.showproject = false;
        this.showpartnerdetails = false;
        this.showmachine = false;
        this.showpagination = false;
      this.showerrmessage = "Partner data is not found.";
      }else{
        this.showpartner = true;
        this.showpartnerdetails = false;
        this.showproject = false;
        this.showmachine = false;
        this.showhideerrmsg = false;
        this.showpagination = true;
        this.pagination();
        this.updateViewAfterPaginationClicked();
      }
      });
      }
    } else if(status === "project"){
      this.set_table_value_project = param;
      this.onchangeproject(param)

      if(param == null || param == ""){

      }else{
        this.httpClient.get<any>("https://rb-mobileweb.de.bosch.com/2021/spm/php_v3.0/material_gettabledata.php?project_id="+param+"&status="+status).subscribe((res: any) =>{
      this.tabledata = res;
      if(this.tabledata == null || this.tabledata == ""){
        this.showhideerrmsg = true;
        this.showpartner = false;
        this.showproject = false;
        this.showpartnerdetails = false;
        this.showmachine = false;
        this.showpagination = false;
        this.showerrmessage = "Project data is not found.";
      }else{
        this.showpartner = false;
      this.showproject = true;
      this.showmachine = false;
      this.showpartnerdetails = false;
      this.showhideerrmsg = false;
      this.showpagination = true;
      this.pagination();
      this.updateViewAfterPaginationClicked();
      }
      });
      }
    } else if(status === "machine"){
      this.set_table_value_machine = param;
      if(param == null || param == ""){
      
      }else{
        this.httpClient.get<any>("https://rb-mobileweb.de.bosch.com/2021/spm/php_v3.0/material_gettabledata.php?machine_id="+param+"&status="+status).subscribe((res: any) =>{
       this.tabledata = res;
      if(this.tabledata == null || this.tabledata == ""){
        this.showhideerrmsg = true;
        this.showpartner = false;
        this.showproject = false;
        this.showpartnerdetails = false;
        this.showmachine = false;
        this.showpagination = false;
        this.showerrmessage = "Machine data is not found.";
      }else{
        this.showpartner = false;
        this.showproject = false;
        this.showpartnerdetails = false;
        this.showmachine = true;
        this.showhideerrmsg = false;
        this.showpagination = true;
        this.pagination();
        this.updateViewAfterPaginationClicked();
        this.tabledata.forEach((element: any) => {
          this.location = element.location;
          this.project_name = element.project_name;
          this.machine_name = element.machine_name;
          this.machine_description = element.machine_description;
          this.machine_phase = element.machine_phase;
          this.industry_sector = element.industry_sector;
          this.application = element.application;
          this.partner_company_name = element.partner_company_name
        });
      }
      });
      }
    }
  }

  pagination() {
    if (this.tabledata.length % this.rowsPerPage != 0) {
      this.endPaging = Math.round(this.tabledata.length / this.rowsPerPage) 
    } else {
      this.endPaging = this.tabledata.length / this.rowsPerPage
    }
  }
  onPageChange(value: any) {
    this.showprevnext = false;
    this.actualPage = value;
    this.updateViewAfterPaginationClicked();
  }
  onChangeRowsPerPage(value: any) {
    // alert(value)
    this.rowsPerPage = value;
    this.pagination();
    this.actualPage = 1;
    this.updateViewAfterPaginationClicked();
  }
  updateViewAfterPaginationClicked() {
    var start = (this.actualPage - 1) * this.rowsPerPage;
    var end = this.actualPage * this.rowsPerPage - 1;
    this.showProducts = this.tabledata.slice(start, end);
  }

  prevnext(){
    this.showprevnext = true;
  }
  // Excel Service
  exportElmToExcel() {
    if(this.showpartnerdetails === true){
      //Excel Title, Header, Data
      const title = 'SPM - Material Planning';
      const header = ["ID", "Partner Company Name","First Name", "Last Name", "Contact Details"]
      
      //Create workbook and worksheet
      let workbook = new Workbook();
      let worksheet = workbook.addWorksheet('Sheet1');

      //Add Row and formatting
      let titleRow = worksheet.addRow([title]);
      titleRow.font = { name: 'BoschSans-Bold', family: 4, vertAlign: 'superscript', size: 24, underline: 'none', bold: true, color: { argb: '002B49' } }
      worksheet.addRow([]);

      worksheet.addRow([]);
      let latestdate = worksheet.addRow([this.latest_date1]);
      latestdate.font = { name: 'BoschSans-Bold', family: 4, size: 12, underline: 'none', bold: true }


      //Add Image
      let logo = workbook.addImage({
        base64: this.logoBase64,
        extension: 'png',
      });
      worksheet.addImage(logo, 'E1:E2');
      worksheet.mergeCells('A1:E3');
      //Blank Row 
      worksheet.addRow([]);
      //Add Header Row
      let headerRow = worksheet.addRow(header);

      // Cell Style : Fill and Border
      headerRow.eachCell((cell, number) => {
        cell.fill = {
          type: 'pattern',
          pattern: 'solid',
          fgColor: { argb: 'FFFFFF' }
        }
        cell.border = { top: { style: 'thin' }, left: { style: 'thin' }, bottom: { style: 'thin' }, right: { style: 'thin' } }
        cell.font = { name: 'BoschSans-Bold', family: 4, size: 12, underline: 'none', bold: false }
      })
      // Add Data and Conditional Formatting
      let i = 1;
      this.tabledata.forEach((d: any) => {
        let row = worksheet.addRow([
          i,
          d.partner_company_name,
          d.first_name,
          d.last_name,
          d.partner_email_address
        ]);
        i = i + 1;
        // let qty = row.getCell(5);
        // let color = 'FF99FF99';
        // if (+qty.value < 500) {
        //   color = 'FF9999'
        // }
        // qty.fill = {
        //   type: 'pattern',
        //   pattern: 'solid',
        //   fgColor: { argb: color }
        // }
        row.font = { name: 'BoschSans-Regular', family: 4, size: 11, underline: 'none', bold: false }
      });

      worksheet.getColumn(2).width = 30;
      worksheet.getColumn(3).width = 20;
      worksheet.getColumn(4).width = 20;
      worksheet.getColumn(5).width = 30;
      worksheet.addRow([]);

      //Footer Row
      let footerRow = worksheet.addRow(['This is a system generated excel sheet. ©Bosch Rexroth AG 2014-2021, all rights reserved ']);
      footerRow.getCell(1).fill = {
        type: 'pattern',
        pattern: 'solid',
        fgColor: { argb: 'FFCCFFE5' }
      };
      footerRow.getCell(1).border = { top: { style: 'thin' }, left: { style: 'thin' }, bottom: { style: 'thin' }, right: { style: 'thin' } }
      //Merge Cells
      worksheet.mergeCells(`A${footerRow.number}:E${footerRow.number}`);
      footerRow.font = { name: 'BoschSans-Regular', family: 4, size: 11, underline: 'none', bold: true }
      //Generate Excel File with given name
      workbook.xlsx.writeBuffer().then((data) => {
        let blob = new Blob([data], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' });
        fs.saveAs(blob, 'MaterialPlanning.xlsx');

      })
    }else if(this.showpartner === true){
      //Excel Title, Header, Data
      const title = 'SPM - Material Planning';
      const header = ["ID", "Solution Partner Name","Customer Title", "Customer Number", "Machine List", "Machine Phase", "Industry Sector", "Application", "Partnumber", "Typecode", "Product Family", "Material Status", "Material Qty", "2019", "2020", "Actual 2021", "Plan 2022", "Plan 2023", "Plan 2024"]
      
      //Create workbook and worksheet
      let workbook = new Workbook();
      let worksheet = workbook.addWorksheet('Sheet1');

      //Add Row and formatting
      let titleRow = worksheet.addRow([title]);
      titleRow.font = { name: 'BoschSans-Bold', family: 4, vertAlign: 'superscript', size: 24, underline: 'none', bold: true, color: { argb: '002B49' } }
      worksheet.addRow([]);

      worksheet.addRow([]);
      let latestdate = worksheet.addRow([this.latest_date1]);
      latestdate.font = { name: 'BoschSans-Bold', family: 4, size: 12, underline: 'none', bold: true }


      //Add Image
      let logo = workbook.addImage({
        base64: this.logoBase64,
        extension: 'png',
      });
      worksheet.addImage(logo, 'S1:S2');
      worksheet.mergeCells('A1:S3');
      //Blank Row 
      worksheet.addRow([]);
      //Add Header Row
      let headerRow = worksheet.addRow(header);

      // Cell Style : Fill and Border
      headerRow.eachCell((cell, number) => {
        cell.fill = {
          type: 'pattern',
          pattern: 'solid',
          fgColor: { argb: 'FFFFFF' }
        }
        cell.border = { top: { style: 'thin' }, left: { style: 'thin' }, bottom: { style: 'thin' }, right: { style: 'thin' } }
        cell.font = { name: 'BoschSans-Bold', family: 4, size: 12, underline: 'none', bold: false }
      })

      // Add Data and Conditional Formatting
      let i = 1;
      this.tabledata.forEach((d: any) => {
        let row = worksheet.addRow([
          i,
          d.partner_company_name,
          d.project_name,
          d.project_number,
          d.machine_name,
          d.machine_phase,
          d.industry_sector,
          d.application,
          d.part_number,
          d.description,
          d.product_family,
          d.material_phase,
          d.qtypermachine,
          d.actual2019,
          d.actual2020,
          d.actual2021,
          d.actual2022,
          d.actual2023,
          d.actual2024
        ]);
        i = i + 1;
        // let qty = row.getCell(5);
        // let color = 'FF99FF99';
        // if (+qty.value < 500) {
        //   color = 'FF9999'
        // }
        // qty.fill = {
        //   type: 'pattern',
        //   pattern: 'solid',
        //   fgColor: { argb: color }
        // }
        row.font = { name: 'BoschSans-Regular', family: 4, size: 11, underline: 'none', bold: false }
      });

      worksheet.getColumn(2).width = 30;
      worksheet.getColumn(3).width = 30;
      worksheet.getColumn(4).width = 30;
      worksheet.getColumn(5).width = 30;
      worksheet.getColumn(6).width = 30;
      worksheet.getColumn(7).width = 30;
      worksheet.getColumn(8).width = 30;
      worksheet.getColumn(9).width = 30;
      worksheet.getColumn(10).width = 30;
      worksheet.getColumn(11).width = 30;
      worksheet.getColumn(12).width = 30;
      worksheet.getColumn(13).width = 15;
      worksheet.getColumn(14).width = 15;
      worksheet.getColumn(15).width = 15;
      worksheet.getColumn(16).width = 15;
      worksheet.getColumn(17).width = 15;
      worksheet.getColumn(18).width = 15;
      worksheet.addRow([]);

      //Footer Row
      let footerRow = worksheet.addRow(['This is a system generated excel sheet. ©Bosch Rexroth AG 2014-2021, all rights reserved ']);
      footerRow.getCell(1).fill = {
        type: 'pattern',
        pattern: 'solid',
        fgColor: { argb: 'FFCCFFE5' }
      };
      footerRow.getCell(1).border = { top: { style: 'thin' }, left: { style: 'thin' }, bottom: { style: 'thin' }, right: { style: 'thin' } }
      //Merge Cells
      worksheet.mergeCells(`A${footerRow.number}:S${footerRow.number}`);
      footerRow.font = { name: 'BoschSans-Regular', family: 4, size: 11, underline: 'none', bold: true }
      //Generate Excel File with given name
      workbook.xlsx.writeBuffer().then((data) => {
        let blob = new Blob([data], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' });
        fs.saveAs(blob, 'MaterialPlanning.xlsx');
      })
    }else if(this.showmachine === true){
      //Excel Title, Header, Data
      const title = 'SPM - Material Planning';
      const header = ["ID", "Machine List", "Part Number", "Typecode", "Product Family", "Deliverytime", "Material Status", "Qty per Machine", "2019", "2020", " 2021", "Actual 2022", "Plan 2023", "Plan 2024"]
      
      //Create workbook and worksheet
      let workbook = new Workbook();
      let worksheet = workbook.addWorksheet('Sheet1');

      //Add Row and formatting
      let titleRow = worksheet.addRow([title]);
      titleRow.font = { name: 'BoschSans-Bold', family: 4, vertAlign: 'superscript', size: 24, underline: 'none', bold: true, color: { argb: '002B49' } }
      worksheet.addRow([]);

      worksheet.addRow([]);
      let latestdate = worksheet.addRow([this.latest_date1]);
      latestdate.font = { name: 'BoschSans-Bold', family: 4, size: 12, underline: 'none', bold: true }


      //Add Image
      let logo = workbook.addImage({
        base64: this.logoBase64,
        extension: 'png',
      });
      worksheet.addImage(logo, 'N1:N2');
      worksheet.mergeCells('A1:N3');
      //Blank Row 
      worksheet.addRow([]);
      //Add Header Row
      let headerRow = worksheet.addRow(header);

      // Cell Style : Fill and Border
      headerRow.eachCell((cell, number) => {
        cell.fill = {
          type: 'pattern',
          pattern: 'solid',
          fgColor: { argb: 'FFFFFF' }
        }
        cell.border = { top: { style: 'thin' }, left: { style: 'thin' }, bottom: { style: 'thin' }, right: { style: 'thin' } }
        cell.font = { name: 'BoschSans-Bold', family: 4, size: 12, underline: 'none', bold: false }
      })

      // Add Data and Conditional Formatting
      let i = 1;
      this.tabledata.forEach((d: any) => {
        let row = worksheet.addRow([
          i,
          d.machine_name,
          d.part_number,
          d.description,
          d.product_family,
          d.quick_delivery_time,
          d.material_phase,
          d.qtypermachine,
          d.actual2019,
          d.actual2020,
          d.actual2021,
          d.actual2022,
          d.actual2023,
          d.actual2024
        ]);
        i = i + 1;
        // let qty = row.getCell(5);
        // let color = 'FF99FF99';
        // if (+qty.value < 500) {
        //   color = 'FF9999'
        // }
        // qty.fill = {
        //   type: 'pattern',
        //   pattern: 'solid',
        //   fgColor: { argb: color }
        // }
        row.font = { name: 'BoschSans-Regular', family: 4, size: 11, underline: 'none', bold: false }
      });

      worksheet.getColumn(2).width = 30;
      worksheet.getColumn(3).width = 30;
      worksheet.getColumn(4).width = 50;
      worksheet.getColumn(5).width = 30;
      worksheet.getColumn(6).width = 30;
      worksheet.getColumn(7).width = 30;
      worksheet.getColumn(8).width = 15;
      worksheet.getColumn(9).width = 15;
      worksheet.getColumn(10).width = 15;
      worksheet.getColumn(11).width = 15;
      worksheet.getColumn(12).width = 15;
      worksheet.getColumn(13).width = 15;
      worksheet.getColumn(14).width = 15;
      worksheet.addRow([]);

      //Footer Row
      let footerRow = worksheet.addRow(['This is a system generated excel sheet. ©Bosch Rexroth AG 2014-2021, all rights reserved ']);
      footerRow.getCell(1).fill = {
        type: 'pattern',
        pattern: 'solid',
        fgColor: { argb: 'FFCCFFE5' }
      };
      footerRow.getCell(1).border = { top: { style: 'thin' }, left: { style: 'thin' }, bottom: { style: 'thin' }, right: { style: 'thin' } }
      //Merge Cells
      worksheet.mergeCells(`A${footerRow.number}:N${footerRow.number}`);
      footerRow.font = { name: 'BoschSans-Regular', family: 4, size: 11, underline: 'none', bold: true }
      //Generate Excel File with given name
      workbook.xlsx.writeBuffer().then((data) => {
        let blob = new Blob([data], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' });
        fs.saveAs(blob, 'MaterialPlanning.xlsx');
      })
    }else if(this.showproject === true){
      //Excel Title, Header, Data
      const title = 'SPM - Material Planning';
      const header = ["ID", "Solution Partner Name","Customer Title", "Customer Number", "Machine List", "Description", "Machine Phase", "Industry Sector", "Application", "Location"]
      
      //Create workbook and worksheet
      let workbook = new Workbook();
      let worksheet = workbook.addWorksheet('Sheet1');

      //Add Row and formatting
      let titleRow = worksheet.addRow([title]);
      titleRow.font = { name: 'BoschSans-Bold', family: 4, vertAlign: 'superscript', size: 24, underline: 'none', bold: true, color: { argb: '002B49' } }
      worksheet.addRow([]);

      worksheet.addRow([]);
      let latestdate = worksheet.addRow([this.latest_date1]);
      latestdate.font = { name: 'BoschSans-Bold', family: 4, size: 12, underline: 'none', bold: true }


      //Add Image
      let logo = workbook.addImage({
        base64: this.logoBase64,
        extension: 'png',
      });
      worksheet.addImage(logo, 'J1:J2');
      worksheet.mergeCells('A1:J3');
      //Blank Row 
      worksheet.addRow([]);
      //Add Header Row
      let headerRow = worksheet.addRow(header);

      // Cell Style : Fill and Border
      headerRow.eachCell((cell, number) => {
        cell.fill = {
          type: 'pattern',
          pattern: 'solid',
          fgColor: { argb: 'FFFFFF' }
        }
        cell.border = { top: { style: 'thin' }, left: { style: 'thin' }, bottom: { style: 'thin' }, right: { style: 'thin' } }
        cell.font = { name: 'BoschSans-Bold', family: 4, size: 12, underline: 'none', bold: false }
      })

      // Add Data and Conditional Formatting
      let i = 1;
      this.tabledata.forEach((d: any) => {
        let row = worksheet.addRow([
          i,
          d.partner_company_name,
          d.project_name,
          d.project_number,
          d.machine_name,
          d.machine_description,
          d.machine_phase,
          d.industry_sector,
          d.application,
          d.location
        ]);
        i = i + 1;
        // let qty = row.getCell(5);
        // let color = 'FF99FF99';
        // if (+qty.value < 500) {
        //   color = 'FF9999'
        // }
        // qty.fill = {
        //   type: 'pattern',
        //   pattern: 'solid',
        //   fgColor: { argb: color }
        // }
        row.font = { name: 'BoschSans-Regular', family: 4, size: 11, underline: 'none', bold: false }
      });

      worksheet.getColumn(2).width = 30;
      worksheet.getColumn(3).width = 30;
      worksheet.getColumn(4).width = 30;
      worksheet.getColumn(5).width = 30;
      worksheet.getColumn(6).width = 40;
      worksheet.getColumn(7).width = 40;
      worksheet.getColumn(8).width = 40;
      worksheet.getColumn(9).width = 40;
      worksheet.getColumn(10).width = 30;
      worksheet.addRow([]);

      //Footer Row
      let footerRow = worksheet.addRow(['This is a system generated excel sheet. ©Bosch Rexroth AG 2014-2021, all rights reserved ']);
      footerRow.getCell(1).fill = {
        type: 'pattern',
        pattern: 'solid',
        fgColor: { argb: 'FFCCFFE5' }
      };
      footerRow.getCell(1).border = { top: { style: 'thin' }, left: { style: 'thin' }, bottom: { style: 'thin' }, right: { style: 'thin' } }
      //Merge Cells
      worksheet.mergeCells(`A${footerRow.number}:J${footerRow.number}`);
      footerRow.font = { name: 'BoschSans-Regular', family: 4, size: 11, underline: 'none', bold: true }
      //Generate Excel File with given name
      workbook.xlsx.writeBuffer().then((data) => {
        let blob = new Blob([data], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' });
         fs.saveAs(blob, 'MaterialPlanning.xlsx');
        
      })
    }
  }

  
}
