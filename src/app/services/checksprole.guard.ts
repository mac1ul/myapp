...
export class ChecksproleGuard implements CanActivate {
    ...
    state: RouterStateSnapshot): Observable<boolean | UrlTree> | Promise<boolean | UrlTree> | boolean | UrlTree {
      if(this.loginservice.gettoken() != null || this.cookieservice.get('token') !== ''){
        ...
        // if (role) {
          ...
          const getauthstatuss: boolean = this.loginservice.isSpartner();

          if (getauthstatuss) {
            console.log('Solution Partner role from auth')
